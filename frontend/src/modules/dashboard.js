...
import { TIPO_IC, TIPO_BG } from '../constants.js';
...
export function renderDash() {
  ...
  if (dcl) {
    dcl.innerHTML = uc.length
      ...
      : '<div class="empty" style="padding:18px"><div class="ei">👥</div><p>Sin clientes</p></div>';
  }
  ...
  if (dp2) {
    dp2.innerHTML = up.length
      ? up.map(p => {
          ...
          const ic = TIPO_IC[p.tipoServicio] || '📦', bg = TIPO_BG[p.tipoServicio] || '#f1f5f9';
          return `<div style="display:flex;align-items:center;gap:9px;padding:8px 0;border-bottom:1px solid var(--bo)">
            <div style="width:32px;height:32px;border-radius:50%;background:${bg};display:flex;align-items:center;justify-content:center;font-size:15px;flex-shrink:0">${ic}</div>
      ...
      : '<div class="empty" style="padding:18px"><div class="ei">📦</div><p>Sin pedidos</p></div>';
  }
}
